...
def advance_and_check(pg, label, action_fn, expect_index):
    ...
    check(label+' pre-flipped', pg.evaluate("()=>sessionState.flipped===true"))
    action_fn()
    # sample immediately and after 130ms
    t0=backTransform(pg)
    idx0=pg.evaluate('()=>sessionState.currentIndex')
    fl=pg.evaluate('()=>sessionState.flipped')
    ...
    curword=pg.evaluate('()=>session[sessionState.currentIndex] && session[sessionState.currentIndex].word')
...
with sync_playwright() as p:
    ...
    idxb=pg.evaluate('()=>sessionState.currentIndex')
    ...
    check('swipePrev flipped=false', pg.evaluate('()=>sessionState.flipped')==False)
    check('swipePrev back not animating', ta==tb)
    check('swipePrev index decreased', pg.evaluate('()=>sessionState.currentIndex')==idxb-1)
    check('swipePrev front word', w==pg.evaluate('()=>session[sessionState.currentIndex].word'))

    # rapid repeated key presses on ONE flipped card -> advances EXACTLY once (no double-grade)
    pg.evaluate("()=>{ if(!document.getElementById('flashcard').classList.contains('flipped')) flipCard(); }"); pg.wait_for_timeout(120)
    idx=pg.evaluate('()=>sessionState.currentIndex')
    for _ in range(5): pg.keyboard.press('3')
    pg.wait_for_timeout(200)
    check('rapid key advances exactly 1', pg.evaluate('()=>sessionState.currentIndex')==idx+1)
    ...
    pg.evaluate("()=>{ if(!document.getElementById('flashcard').classList.contains('flipped')) flipCard(); }"); pg.wait_for_timeout(120)
    idx2=pg.evaluate('()=>sessionState.currentIndex')
    for _ in range(5): pg.evaluate("()=>document.querySelector('.rate.good').click()")
    pg.wait_for_timeout(150)
    check('rapid click advances exactly 1', pg.evaluate('()=>sessionState.currentIndex')==idx2+1)
