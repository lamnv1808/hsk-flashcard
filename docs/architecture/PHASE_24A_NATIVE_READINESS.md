...
- **Accessibility:** `role="dialog"` + `aria-modal="true"` + labelled heading; real `<label for>`; intentional initial focus (first field); focus trap (Tab cycles); Escape cancels (no request); errors in an `aria-live="assertive"` region; ≥44px buttons; background interaction blocked (`body.auth-locked`).
...
### Review follow-up (same phase; SW stays v34)
- **Duplicate-submit guard (Finding 1):** a modal-local `submitting` flag makes `submit()` idempotent while pending — the first valid submission proceeds; every additional click **or Enter press** (Enter listeners call `submit()` directly) is ignored, so exactly one `onSubmit`/Edge request occurs. Both submit and cancel buttons are disabled while pending; **Escape is a no-op while a request is in flight** (no second action / inconsistent state). On validation or server error the guard resets and the buttons re-enable (retryable); on success the modal closes via the existing flow. No timers, no global state; Edge calls/payloads unchanged.
- **Focus restoration (Finding 2):** the modal now restores focus to **`#profileBtn`** (the stable account control), resolved at close time, instead of the `body` that `document.activeElement` yielded (the profile menu self-hides before the modal opens). Safe fallback when `#profileBtn` is absent/disconnected (e.g. after delete+reload): no forced focus. Never focuses a hidden menu item; profile-menu behavior unchanged.
