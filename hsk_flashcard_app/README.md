...
- Cài đặt âm thanh: tốc độ 0.5x / 0.75x / 1x / 1.25x / 1.5x, tự đọc từ khi hiện thẻ mới, tự đọc ví dụ sau khi lật, nút **Dừng**.
...
- Có **tên hiển thị** (giữ nguyên chữ hoa/thường) trong khi so sánh trùng lặp/đăng nhập vẫn theo chữ thường; cổng đăng nhập có nút **Hiện mã PIN**.

## V6 — Hiển thị Pinyin ở mặt trước (per-user)
- Cài đặt mới **"Hiển thị Pinyin ở mặt trước"** (mục *Hiển thị* ở trang chủ). **Mặc định bật** → giữ nguyên hành vi hiện tại cho người dùng cũ.
- **Bật**: mặt trước hiện chữ Hán (cột B) + pinyin từ vựng (cột C); mặt sau như cũ (không lặp pinyin từ vựng).
- **Tắt**: mặt trước chỉ hiện chữ Hán; pinyin từ vựng chuyển sang **đầu mặt sau** (chữ Hán + pinyin từ vựng) rồi mới đến nghĩa/ví dụ. Pinyin ví dụ luôn hiển thị; pinyin từ vựng không bao giờ mất và không xuất hiện ở cả hai mặt.
- Áp dụng ngay, không cần tải lại. Lưu trong cấu hình cài đặt hiện có; khi đăng nhập thì đồng bộ theo từng tài khoản (hai người dùng có thể có tùy chọn khác nhau). Không đọc pinyin (audio vẫn chỉ đọc tiếng Trung).
